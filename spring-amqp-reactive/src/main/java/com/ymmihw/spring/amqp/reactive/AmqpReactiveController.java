...
import com.ymmihw.spring.amqp.reactive.DestinationsConfig.DestinationInfo;
...
public class AmqpReactiveController {
  ...
  public void setupQueueDestinations() {
    ...
    destinationsConfig.getQueues().forEach((key, destination) -> {

      log.info("[I54] Creating directExchange: key={}, name={}, routingKey={}", key,
      ...
      Binding b = BindingBuilder.bind(q).to(ex).with(destination.getRoutingKey()).noargs();
  ...
  public void setupTopicDestinations() {
    ...
    destinationsConfig.getTopics().forEach((key, destination) -> {
      ...
      log.info("[I98] Creating TopicExchange: name={}, exchange={}", key,
          destination.getExchange());

      Exchange ex = ExchangeBuilder.topicExchange(destination.getExchange()).durable(true).build();
  ...
  public Mono<ResponseEntity<?>> sendMessageToQueue(@PathVariable String name,
    ...
    return Mono.fromCallable(() -> {
      ...
      log.info("[I51] sendMessageToQueue: queue={}, routingKey={}", d.getExchange(),
          d.getRoutingKey());
  ...
  public Flux<?> receiveMessagesFromQueue(@PathVariable String name) {
    ...
    MessageListenerContainer mlc =
        messageListenerContainerFactory.createMessageListenerContainer(d.getRoutingKey());
    ...
    return Flux.interval(Duration.ofSeconds(5)).map(v -> {
  ...
  public Mono<ResponseEntity<?>> sendMessageToTopic(@PathVariable String name,
      @RequestBody String payload) {
    ...
    return Mono.fromCallable(() -> {
      ...
      log.info("[I51] sendMessageToTopic: topic={}, routingKey={}", d.getExchange(),
          d.getRoutingKey());
      amqpTemplate.convertAndSend(d.getExchange(), d.getRoutingKey(), payload);
